...
class BasicBlock(nn.Module):
    ...
    def forward(self, input_x):
        ...
        out = self.conv1(x, alphas)
        out = self.bn1(out)
        out = self.relu(out)

        out = self.conv2(out, alphas)
        out = self.bn2(out)
...
class ResNet(nn.Module):
    ...
    def forward(self, x, alphas=None, return_intermediate_features=False, norm_flag=False):
        x = self.conv1(x, alphas)
        x = self.bn1(x)
        x = self.relu(x)
        x1 = self.maxpool(x) 
        
        x2, x2_pre = self.layer1({0:x1, 1:alphas})
        x3, x3_pre = self.layer2({0:x2, 1:alphas})
        x4, x4_pre = self.layer3({0:x3, 1:alphas})
        x5, x5_pre = self.layer4({0:x4, 1:alphas})
