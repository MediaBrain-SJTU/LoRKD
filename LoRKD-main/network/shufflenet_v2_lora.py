...
class InvertedResidual(BaseModule):
    ...
    def forward(self, input_x):
        ...
        def _inner_forward(input_x):
            x, alphas = input_x[0], input_x[1]
            if self.stride > 1:
                # for th, each_conv in enumerate(self.branch1):
                branch1_out = self.branch1[0](x)
                branch1_out = self.branch1[1]({0:branch1_out, 1:alphas})
                # for th, each_conv in enumerate(self.branch2):
                ...
                # out = torch.cat((self.branch1({0:x, 1:alphas})[0], self.branch2({0:x, 1:alphas})[0]), dim=1)
            else:
...
class ShuffleNetV2(BaseBackbone):
    ...
    def forward(self, x, alphas=None, return_intermediate_features=False, norm_flag=False):
        x = self.conv1({0:x, 1:alphas})
        x = self.maxpool(x)
        ...
        for i, layer in enumerate(self.layers):
            ...
            if layer.__class__ == ConvModule:
                # print('3: ', i, x.shape, layer.__class__)
                x = layer({0:x, 1:alphas})#[0]
            else:
                # print('1: ', i, x.shape, len(layer), layer.__class__)
                for th, each_block in enumerate(layer):
                    # print('2: ', th, x.shape, each_block.__class__)
                    # for ith, each_conv in enumerate(each_block):
                        # print('4 :', ith, each_conv.__class__)
                    x = each_block({0:x, 1:alphas})
            # x = layer({0:x, 1:alphas})
