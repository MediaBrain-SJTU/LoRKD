...
from data.medmnist import MedMnistData
...
def main():
    ...
    num_classes = [9, 4, 11, 8, 2, 8, 7, 11, 11, 2]
    total_num_classes = 73
    num_task = 10
    ...
    checkpoint_file = 'pretrained_resnet50_224_medmnist.pth'
    checkpoint = torch.load(checkpoint_file)
    checkpoint.pop('res_fc.module.weight')
    checkpoint.pop('res_fc.module.bias')
    image_state_dict = checkpoint
    new_image_state_dict = OrderedDict()
    for k, v in image_state_dict.items():
            k = k.split('multi_task_model.')[1]
            new_image_state_dict[k] = v 
    ...
    teacher_model['backbone'].load_state_dict(new_image_state_dict, strict = True)
    ...
    # data
    train_dataset = MedMnistData(split="train")
    test_dateset = MedMnistData(split="test")
    ...
    torch.save(student_model.state_dict(), 'lora_medmnist_shufflenet.pth')
